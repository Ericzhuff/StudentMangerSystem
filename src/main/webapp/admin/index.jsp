...
<body class="main_body" basePath="<s:url value="/"/>">
				...
				<ul class="layui-nav top_menu">
					...
					<li class="layui-nav-item" pc><a href="javascript:;"
						style="pointer-events: none;"> <img
							src="<s:url value='/upload/' /><s:property value='#session.image' />"
							class="layui-circle faceImage" width="35" height="35"> <cite><s:property
									value="#session.name" /></cite>
					</a>
		...
		<div class="layui-side layui-bg-black">
			<div class="user-photo">
				<a class="img" title="我的头像"><img class="layui-anim faceImage"
					...
					src="<s:url value='/upload/' /><s:property value='#session.image' />"></a>
				<p>
					你好！<span class="name"><s:property value="#session.name" /></span>, 欢迎登录 <input
						type="hidden" class="id" value="<s:property value="#session.id" />" />
				</p>
				<input type="hidden" class="tch_id" value="<s:property value="#session.tch_id" />">
				<input type="hidden" class="type"
					value="<s:property value='#session.type' />" /> <input type="hidden"
					class="userName" value="<s:property value='#session.userName' />" />
			</div>
	...
	<div class="admin-header-lock" id="lock-box" style="display: none;">
		<div class="admin-header-lock-img">
			<img src="<s:url value='/upload/' /><s:property value='#session.image' />" />
		</div>
	...
	<script>
		var navs = $.ajax({
			...
			"success" : function(data) {
				if(data.length > 100){
					location.href = "<s:url value='/admin/login/login.jsp' />";
				}else{
					var navs = [];
		...
		html = "";
		if ("<s:property value='#session.type'/>" == "student") {
			html = '<a href="javascript:;" data-url="<s:url value='/studentInfo/perInfoedit' />?id=<s:property value='#session.id' />"><i class="iconfont icon-zhanghu" data-icon="icon-zhanghu"></i><cite>个人资料</cite></a>';
		}else if("<s:property value='type'/>" == "teacher"){
			html = '<a href="javascript:;" data-url="<s:url value='/teacher/perInfoedit' />?id=<s:property value='#session.id' />"><i class="iconfont icon-zhanghu" data-icon="icon-zhanghu"></i><cite>个人资料</cite></a>';
		} else {
			html = '<a href="javascript:;" data-url="<s:url value='/Admin/edit' />?id=<s:property value='#session.id' />"><i class="iconfont icon-zhanghu" data-icon="icon-zhanghu"></i><cite>个人资料</cite></a>';
		}
		$(".info").append(html)
	</script>
	<script type="text/javascript"

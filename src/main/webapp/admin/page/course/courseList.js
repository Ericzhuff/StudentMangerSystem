layui.config({
	...
	$("body").on("click",".course_view",function(){  //详情
		var _this = $(this);
		var index = layui.layer.open({
			title : "查看课程信息",
			type : 2,
			content : basePath+"/course/view?id="+_this.attr("data-id"),
			success : function(layero, index){
				layui.layer.tips('点击此处返回课程信息列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
			    	...
			    	+'<td>'
			    	+  '<a class="layui-btn layui-btn-normal layui-btn-mini course_view" data-id="'+data[i].id+'"><i class="layui-icon">&#xe60b;</i> 详细</a>'

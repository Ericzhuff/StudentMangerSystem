layui.config({
	...
	function LoadData(){
		$.get(basePath+"course/findById?id="+$("#id").val(), function(data){
	        	newsList(data.student,data.course_name,data.course_id,data.id);
	        	$.ajax({
		...
		function renderDate(data,curr){
			...
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
					dataHtml += '<tr>'
			    	+'<td><input type="checkbox" class="id" name="checked" lay-skin="primary" lay-filter="choose" data-id="'+id+'"></td>'

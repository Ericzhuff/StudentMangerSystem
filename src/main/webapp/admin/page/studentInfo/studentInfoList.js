layui.config({
	...
	function LoadData(){
		var newsData = '';
		$.get(basePath+"studentInfo/list", function(data){
	        	newsData = data;
	        	newsList(newsData);
	        	var search = [];
	        	for(var i = 0;i<data.length;i++){
	        		search.push(data[i].stu_id.trim());
	        		search.push(data[i].stu_name.trim());
	        	}
	...
	$(".search_btn").click(function(){
		...
		if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
            	$.ajax({
					url : basePath+"studentInfo/findByKeyWords",
					type : "post",
					dataType : "json",
		...
		if($checkbox.is(":checked")){
			layer.confirm('确定删除选中的信息？',{icon:3, title:'提示信息'},function(index){
	            ...
	            setTimeout(function(){
	            	...
	            	$.ajax({
	    				"url":basePath+"studentInfo/delete",
	...
	$("body").on("click",".stu_edit",function(){  //编辑
		...
		var index = layui.layer.open({
			title : "修改学生信息",
			type : 2,
			content : basePath+"studentInfo/edit?id="+_this.attr("data-id"),
			success : function(layero, index){
				layui.layer.tips('点击此处返回学生信息列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});
	...
	$("body").on("click",".stu_view",function(){  //详细
		...
		var index = layui.layer.open({
			title : "查看学生信息",
			type : 2,
			content : basePath+"studentInfo/view?id="+_this.attr("data-id"),
			success : function(layero, index){
				layui.layer.tips('点击此处返回学生信息列表', '.layui-layer-setwin .layui-layer-close', {
					tips: 3
				});

layui.config({
	...
	function LoadData(){
		var newsData = '';
		$.get(basePath+"teacher/list", function(data){
	        	newsData = data;
	        	newsList(newsData);
	        	var search = [];
	        	for(var i = 0;i<data.length;i++){
	        		search.push(data[i].tch_id.trim());
	        		search.push(data[i].tch_name.trim());
	        	}
	...
	$(".search_btn").click(function(){
		...
		if($(".search_input").val() != ''){
            ...
            setTimeout(function(){
            	$.ajax({
					url : basePath+"teacher/findByKeyWords",
					type : "post",
					dataType : "json",
	...
	$("body").on("click",".stu_edit",function(){  //编辑
		...
		var index = layui.layer.open({
			title : "修改教师信息",
			type : 2,
			content : basePath+"teacher/edit?id="+_this.attr("data-id"),
			success : function(layero, index){
	...
	$("body").on("click",".stu_view",function(){  //详细
		...
		var index = layui.layer.open({
			title : "查看教师信息",
			type : 2,
			content : basePath+"teacher/view?id="+_this.attr("data-id"),
			success : function(layero, index){
